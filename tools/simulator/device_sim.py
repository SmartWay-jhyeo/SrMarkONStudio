    ...
    def _on_stat(self, _args: tuple[str, ...]) -> list[str]:
        ...
        return [
            self._json(
                ...
                #
                #    `link` 가 "선이 몇 bps 인가" 라면 이것은 "그 선에 대기줄이
                #    얼마나 밀려 있나" 다. 실기기에서 카탈로그(103줄 ≈ 25 KB)가
                #    4,096 B 짜리 링을 넘겨 43줄만 도착했고, 그 사실이 전선에
                #    없어서 GDB 로 `p 'mk_uart.c'::s_tx` 를 해서야 찾았다.
                #
                #    [단순화, 시뮬레이터] 여기에는 링이 없다. 소켓이 알아서
                #    버퍼링하고 한 줄도 안 버린다. 그러니 **지어내지 않는다** —
                #    통째로 null 이다("이 장치는 답할 수 없다"). 0 을 채우면
                #    "링이 있는데 한 번도 안 찼다" 로 읽혀 진단 화면이 거짓
                #    안심을 준다. clock 과 같은 판단이고, 펌웨어도 링을 안
                #    붙인 빌드에서 정확히 이 값을 낸다(mk_cfgwire.c).
                tx=None,
