...
🔴 **`ain`·`i2c`는 같은 값이 반복될 수 있다** (사용자 설계 2026-08-19).
보드는 수집과 송신을 뗀다 — 채널·포트마다 "마지막으로 읽은 값" 하나만
들고 있다가, `tx.period_ms`마다 그것을 그대로 내보낸다(`i2c`는 포트마다
별도의 `i2cN.period_ms`로 수집하지만, 내보내는 주기는 `ain`과 똑같이
`tx.period_ms`다). 그 사이 새로 수집된 것이 없으면 **직전과 완전히 같은
값이 다시 나간다** — 채널이 죽어서가 아니라 설계상 정상이다.

이때 **`t`는 송신 시각이 아니라 그 값을 실제로 읽은 획득 시각 그대로다.**
반복돼도 `t`는 바뀌지 않는다 — 이것이 이 설계가 성립하는 유일한 이유다.
...
수집이 송신보다 빠르면 그 사이 표본은 조용히 버려지고 최신 값만 나간다
