...
{"schema_ver":3,"seq":1234,"t":1772200855875,"type":"i2c","connector_id":10,
 "quantity":"temp","value":23.41,"unit":"°C","status":0}
```
...
`time_quality`·`unit`·`status`·`connector_id` 비트가 그대로 걸린다.
...
| `temp` | 온도 | `°C` |
| `humidity` | 상대습도 | `%RH` |
| `temp_object` | 적외선 대상 온도 | `°C` |
| `temp_ambient` | 적외선 주변 온도 | `°C` |
| `pressure` | 기압 | `hPa` |

같은 포트가 `temp`와 `humidity`를 함께 낼 수 있다. 대시보드는
(`connector_id`, `quantity`) 짝마다 카드 하나를 그린다.
