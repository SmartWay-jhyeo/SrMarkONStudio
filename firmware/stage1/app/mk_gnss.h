 ...
 *    7자리인데, 십진도 경도는 `127.34059070` 로 11자리다 — float 에 넣으면
 ...
 *
 *    그래서 **1e-8 도 단위 정수**로 담는다. 1e-8 도 = 적도에서 약 1.11 mm 라
 *    RTK 의 정밀도보다도 세밀하다 — 실기기 문장(UM981, 2026-08-20)이 분에
 *    소수 8자리(`mm.mmmmmmmm`)를 그대로 주므로, 도로 환산해도(÷60) 정보
 *    손실 없이 8자리를 채울 수 있다(mk_gnss.c 의 mk_gnss_ddmm_to_1e8 주석).
 *
 *    🔴 [확장, 2026-08-20] 예전에는 1e-7 도였고 경도 최댓값 1,800,000,000
 *    이 int32(2,147,483,647) 안에 들어갔다. 1e-8 로 올리면 경도 최댓값이
 *    18,000,000,000(180 × 10^8) 이라 int32 를 크게 넘는다 — 그래서
 *    `lat_1e8`·`lon_1e8` 은 **int64_t** 다. 도-분 -> 십진도 변환도 정수
 *    산술만으로 한다(mk_gnss.c 의 mk_gnss_ddmm_to_1e8).
 *
...
typedef struct {
     ...
     * int64_t 다 — 1e-8 도 스케일에서 경도 최댓값(18,000,000,000)이
     * int32 를 넘는다(위 헤더 주석). */
    int     have_pos;
    int64_t lat_1e8;
    int64_t lon_1e8;
 ...
 *
 * 🔴 `int64_t` 다 — 경도 ±180° × 10^8 = ±180억이 int32 상한(약 21억)을
 *    넘는다(mk_gnss.c 의 mk_gnss_ddmm_to_1e8 주석).
 *
