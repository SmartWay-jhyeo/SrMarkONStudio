 ...
 *    성립한다.
 *
 *    16 이라는 값 자체는 이번에 올리지 않았다 — 정상 부하에서 켜진
 *    채널이 최대 MK_ADS_CHANNELS(7)개면 한 틱에 최소 7줄이 필요한데,
 *    16 이면 라운드로빈 공정 몫이 채널당 최소 floor(16/7)=2줄이라
 *    (채널당 틱마다 표본 1개, 10ms 주기 가정) 수집 속도보다 항상 빠르게
 *    비운다 — 밀렸던 만큼은 몇 틱 안에 줄어들지 늘지 않는다. 값을
 *    올리면 mk_uart_write 가 블로킹으로 그만큼 더 오래 잡아먹어(main.c 의
 *    SAMPLES_PER_CHANNEL 주석에 있는 134ms 계산이 그대로 커진다) 슈퍼루프
 *    한 바퀴가 더 늘어지는 역효과가 있다 — 이번 문제는 예산 부족이
 *    아니라 공정성 문제였다. */
