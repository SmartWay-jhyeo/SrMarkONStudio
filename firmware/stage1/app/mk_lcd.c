...
static const MkLcdCmd INIT_CMDS[] = {
     ...
     *
     *    이 한 비트를 세우면 mk_lcd_pixel() 이 만든 R·G·B 순서가 패널의
     *    필터 순서와 맞는다 — 화소 변환 쪽을 뒤집지 않는 이유는, 뒤집는
     *    곳이 둘이 되면 나중에 한쪽만 고쳐 놓고 왜 색이 도로 틀렸는지 못
     *    찾게 되기 때문이다(mk_solctl 의 flip_polarity 와 같은 이유로
     *    반전은 한 곳에서만 한다). */
