 ...
 *    한 바퀴에 최악 60 ms 라, 슈퍼루프가 시작 신호를 쥐고 있으면 채널당
 *    10 ms 가 구조적으로 불가능하다. (2026-08-20 까지는 텔레메트리의
 *    HAL_UART_Transmit 도 블로킹이라 여기에 더해졌다 — 지금은 링버퍼+DMA
 *    로 걷어냈지만, 남은 60 ms 하나만으로도 결론은 같다.) 이것이 빠지면 컴파일도 부팅도 되고 값도
 *    나오지만, 표본이 조용히 사라진다. */
