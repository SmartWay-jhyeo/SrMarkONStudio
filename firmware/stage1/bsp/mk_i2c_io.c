 ...
 *    이 60 ms 동안에도 계속 돈다.
 *
 *    🔴 [정정 2026-08-19] 뒤 문장은 **예전에는 사실이 아니었다.** 변환은
 *       인터럽트로 돌았지만 다음 채널을 **시작**하는 것은 슈퍼루프의
 *       mk_ads_tick() 뿐이었다. 그래서 이 60 ms 동안 새 변환이 시작되지
 *       않았고, finish() 가 따라잡기를 포기하므로 그 표본은 큐의 drops
 *       에도 안 잡힌 채 사라졌다.
 *
 *       지금은 진행이 전부 인터럽트에 있다 — TIM7(1 kHz)이 쉬던 상태머신을
 *       깨우고, SPI 완료 인터럽트 안의 finish() 가 밀린 다음 채널로 곧바로
 *       이어 간다(bsp/mk_ads_io.c 의 MK_ADS_TICK_HZ). 그래서 이제 정말로
 *       표본을 잃지 않는다. 채널당 10 ms 를 쓰려면 반드시 그래야 한다 —
 *       7채널 × 10 ms 면 채널 하나에 1.43 ms 이고, 이 60 ms 한 번은 그
 *       예산을 42배 넘긴다.
 *
