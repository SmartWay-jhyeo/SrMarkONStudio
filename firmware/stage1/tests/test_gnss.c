 ...
 *    못박았다:
 *        3719.14416560 N -> 373190694  (37.3190694°,  참값 37.31906942666…)
 *        12720.43544199 E -> 1273405907 (127.3405907°, 참값 127.34059069983…)
 */
...
{
    int32_t v = 0;
    CHECK(mk_gnss_ddmm_to_1e7("3719.14416560", 13, 'N', 1, &v) == 1 &&
          v == 373190694, "위도 3719.14416560 N = 373190694 (37.3190694도)");
    CHECK(mk_gnss_ddmm_to_1e7("12720.43544199", 14, 'E', 0, &v) == 1 &&
          v == 1273405907, "경도 12720.43544199 E = 1273405907 (127.3405907도)");

    /* 🔴 되돌림 검사 — 도-분을 그대로 십진도로 읽으면 3719.144… 다.
     *    그 실수를 하면 위 기대값과 자릿수부터 다르다. */
    CHECK(v != 1272043544, "도-분을 그대로 읽지 않는다");

    /* 남위·서경은 음수. */
    CHECK(mk_gnss_ddmm_to_1e7("3719.14416560", 13, 'S', 1, &v) == 1 &&
          v == -373190694, "S 는 음수");
    CHECK(mk_gnss_ddmm_to_1e7("12720.43544199", 14, 'W', 0, &v) == 1 &&
          v == -1273405907, "W 는 음수");

    /* 분이 0 이면 도만 남는다 — 자릿수를 가르는 경계. */
    CHECK(mk_gnss_ddmm_to_1e7("0000.00000000", 13, 'N', 1, &v) == 1 && v == 0,
          "0000.0 = 적도");
    CHECK(mk_gnss_ddmm_to_1e7("9000.0", 6, 'N', 1, &v) == 1 && v == 900000000,
          "9000.0 = 북극(위도 상한)");
    CHECK(mk_gnss_ddmm_to_1e7("18000.0", 7, 'E', 0, &v) == 1 && v == 1800000000,
          "18000.0 = 날짜변경선(경도 상한)");

    /* 범위 밖·형식 오류는 거부한다 — 지어내지 않는다. */
    int32_t before = 12345;
    v = before;
    CHECK(mk_gnss_ddmm_to_1e7("9100.0", 6, 'N', 1, &v) == 0 && v == before,
          "위도 91도는 거부");
    CHECK(mk_gnss_ddmm_to_1e7("18100.0", 7, 'E', 0, &v) == 0 && v == before,
          "경도 181도는 거부");
    CHECK(mk_gnss_ddmm_to_1e7("", 0, 'N', 1, &v) == 0 && v == before,
          "빈 필드는 거부");
    CHECK(mk_gnss_ddmm_to_1e7("19.1", 4, 'N', 1, &v) == 0 && v == before,
          "정수부가 3자리 미만이면 거부(ddmm 이 아니다)");
    CHECK(mk_gnss_ddmm_to_1e7("3719.14416560", 13, 'X', 1, &v) == 0 &&
...
static void test_real_device_fix_record(void)
{
    ...
    CHECK(fix.lat_1e7 == 373190694, "위도 37.3190694");
    CHECK(fix.lon_1e7 == 1273405907, "경도 127.3405907");
...
static void test_fix_record_needs_no_date(void)
{
    ...
    CHECK(fix.have_pos == 1 && fix.lat_1e7 == 373190694, "위치는 그대로");
